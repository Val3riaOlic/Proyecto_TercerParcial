...
NodoBinario* insertarPorClave(NodoBinario* raiz, Parada dato) {
    if (raiz == NULL) return nuevoNodoBinario(dato);
    if (dato.clave < raiz->info.clave) raiz->izquierda = insertarPorClave(raiz->izquierda, dato);
    else if (dato.clave > raiz->info.clave) raiz->derecha = insertarPorClave(raiz->derecha, dato);
    return raiz;
}

NodoBinario* armarArbolParadas(const MapaRuta& red) {
    NodoBinario* raiz = NULL;
    for (int i = 0; i < (int)red.paradas.size(); i++) raiz = insertarPorClave(raiz, red.paradas[i]);
    return raiz;
}

void recorridoPreorden(NodoBinario* raiz) {
    if (raiz == NULL) return;
    cout << "  [" << raiz->info.clave << "] " << raiz->info.nombre << endl;
...
void mostrarRecorridos(const MapaRuta& red) {
    NodoBinario* raiz = armarArbolParadas(red);
    cout << "\n+--------------------------------------------------+" << endl;
    cout << "| RECORRIDOS DEL ARBOL DE PARADAS                 |" << endl;
    cout << "+--------------------------------------------------+" << endl;
    cout << "\nPreorden:" << endl;
